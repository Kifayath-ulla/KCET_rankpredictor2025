...
function predictRankKCET() {
  ...
  if (!boardMarks || !kcetMarks) {
      showPopup('Both fields are mandatory. Please fill in all fields.');
      return;
  }

  if (isNaN(boardMarks) || boardMarks < 0 || boardMarks > 300) {
      boardInput.classList.add('invalid');
      showPopup('Please enter valid board marks (0-300).');
      return;
  }
  if (isNaN(kcetMarks) || kcetMarks < 0 || kcetMarks > 180) {
      kcetInput.classList.add('invalid');
      showPopup('Please enter valid KCET marks (0-180).');
      return;
  }

  hidePopup();

  // Show loading message for 1.5 seconds
  showLoadingMessage();
  setTimeout(() => {
      ...
      // If exact match not found, interpolate rank
      if (predictedRank === -1) {
          if (low === 0) {
      ...
      // Display the predicted rank with the range
      rankBox.style.display = 'block';
      rankBox.innerHTML = `
